...
extractItemsTotals <- function(killList) {
  ...
  for (i in killList) { # something breaks here because of names if the first result has no items dropped. Should check for that first
    rawItems<- rbind(rawItems, cbind(  #Modules, rigs and cargo
    ...
    ShipType<-as.numeric(xmlDoc(i)[["//victim/@shipTypeID"]][1]) # and the ship they were flying
